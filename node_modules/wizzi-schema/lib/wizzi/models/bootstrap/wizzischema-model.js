...
// Node class
Node = function(wzName) {
    this.wzName = wzName;
    this.wzId = schemautil.getIdentifier(wzName);
    this.comments = [];
}
...
Attribute.prototype.wzVerify = function(ctx) {
    this.wzId = schemautil.getIdentifier(this.wzName);
...
// EnumFacet class
EnumFacet = function(name) {
    Facet.call(this, name);
    this.wzElement = 'enumFacet';
}
util.inherits(EnumFacet, Facet);
...
Include.prototype.wzInitialize = function(ctx) {
    ...
    this.actualNameId = schemautil.getIdentifier(this.actualName);
    this.rootEntityNameId = schemautil.getIdentifier(this.rootEntityName);
}
...
Element.prototype.needsVerify = function() {
...
Element.prototype.needsInitialize = function(emitKey) {
    ...
    for (i=0; i<i_len; i++) {
        ...
        if (method.emitKey == emitKey && method.wzName.toLowerCase() === "wzinitialize") {
            return true;
        }
    }
    return false;
...
Element.prototype.setupNames = function(ctx) {
    ...
    this.wzId = schemautil.getIdentifier(this.wzName);
    this.wzIdPlural = schemautil.getIdentifier(this.wzNamePlural);
    this.wzIdCapital = _.capitalize(this.wzId);
    this.tagName = this.tagInternal || this.wzId;
    this.tagSafe = this.tagName.replace("\"", "\\\"");
    this.xmlTag = this.xmlTagInternal || this.wzId;
    if (schemautil.isNotEmpty(this.acceptAnyTag)) {
        this.acceptAnyTagId = schemautil.getIdentifier(this.acceptAnyTag);
    }
    if (schemautil.isNotEmpty(this.acceptAnyTagCollection)) {
        this.acceptAnyTagCollectionId = schemautil.getIdentifier(this.acceptAnyTagCollection);
    }
}
Element.prototype.wzSetup = function(ctx) {
    ...
    if (schemautil.isNotEmpty(this.super)) {
        this.superId = schemautil.getIdentifier(this.super);
            ...
            if (relation.rightElement.addToChildren) {
                ...
                for (j=0; j<j_len; j++) {
                    ...
                    for (k=0; k<k_len; k++) {
                        ...
                        choiceSet.addChoice(test, function(codelines) {
                            codelines.push("item = new " + CHILD_ENTITY + "(child.v, new wzSourceLineInfo(child.row, child.col, child.source))");
...
Relation.prototype.wzInitializeNames = function(ctx) {
    if (this.isOneToOne == false) {
        if (this.childRole == null) {
            ...
            name = hyphen >= 0 ? schemautil.replaceAll(this.wzName, '[-]', '§§§') : this.wzName;
            ss = name.split('/');
            if (ss.length == 2) {
                var zz = ss[0].split('-');
                if (zz.length == 2) {
...
Relation.prototype.wzInitialize = function(ctx) {
    ...
    if (this.rightElement === null) {
        ctx.error('Related element ' + this.wzName + ' not found');
    }
...
// ChoiceOrdered class
ChoiceOrdered = function(char, length) {
    this.char = char;
    this.length = length;
    this.choices = [];
}
...
util.inherits(Require, Node);


md.Require = Require;
// ExportTo class
ExportTo = function(name) {
    Node.call(this, name);
    this.wzElement = 'exportTo';
}
util.inherits(ExportTo, Node);
...
util.inherits(WizziSchema, Node);
...
WizziSchema.prototype.analize = function() {
    ...
    for (i=0; i<i_len; i++) {
        ...
        for (j=0; j<j_len; j++) {
            ...
            else if (attribute.type == md.Attribute.INT) {
                this.addRequire("parseint", 'function');
            }
...
WizziSchema.prototype.loadFromWizziModel = function(wizziModel) {
    ...
    for (i=0; i<i_len; i++) {
        ...
        for (j=0; j<j_len; j++) {
            ...
            attribute.default = aFrom.xdefault;
