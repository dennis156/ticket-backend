$group
	$params &element

	$if element.includes.length > 0
		
		set ok = md.${element.super}.prototype.loadChild.call(this, child)
		$foreach include in element.includes
			
			if !ok
				var ${include.actualNameLower} = new md.${include.actualNameId}.Model.${include.rootEntityNameId}();");
				set ok = ${include.actualNameLower}.loadChild(child);
				if ok
					_ this.${include.actualNameId}Included.push(${include.actualNameLower})
		
		return ok

	$else
		
		$if verify.isNotEmpty(element.super)
			set ok = md.${element.super}.prototype.loadChild.call(this, child)
			
