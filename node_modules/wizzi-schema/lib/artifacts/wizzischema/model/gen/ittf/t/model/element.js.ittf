$group
	...
	class ${element.wzId}
		...
		ctor
			...
			$foreach include in element.includes
				...
				set this.${include.wzId}Included = []
		...
		$foreach facet in element.getEnumFacets()
			$if facet.wzId.toLowerCase() !== "@@empty"
				set md.${element.wzId}.NAME_${facet.wzId.toUpperCase()} = "${facet.wzNameEscaped}"
				...
				$if attribute.allowedDeclaration() != null
					$foreach facet in attribute.getEnumFacets()
						$if facet.wzName.toLowerCase() !== "@@empty"
							set md.${element.wzId}.${attribute.wzId.toUpperCase()}_${facet.wzId.toUpperCase()} = "${facet.wzNameEscaped}"
			...
			$if relation.isOneToOne == false
				...
				$if !relation.rightElement.isAbstract
					...
					m add${relation.childEntityIdCapital}
						arg name
						arg sourceLineInfo
						var retval = new md.${relation.childEntityId}(name, sourceLineInfo)
	...
	set md.${element.wzId} = ${element.wzId}