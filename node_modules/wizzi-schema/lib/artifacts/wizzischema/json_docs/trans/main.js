...
var jsModelType = require('wizzi-legacy').jsModelType;
...
md.trans = function(model, ctx, callback) {
    if (model.wzElement != 'wizzischema') {
        error.error(myname + '.trans. Model wzElement must be wizzischema. Found: ' + model.wzElement, model);
    }
    ctx.result = {
        name: model.wzName, 
...
function normalizeJSTNode(jstnode) {
    ...
    if (jsModelType.__tagEntityMapping[jstnode.n]) {
        node.name = jsModelType.__tagEntityMapping[jstnode.n];
    }
    if (jstnode.childs) {
