/*
    ...
    Input: an artifact config info (constructor injected).
    Output: the artifact config info enriched with (one or many) executed genWriter(s), 
...
Runner.prototype._runOnModelInfo = function (modelInfo, callback) {
    ...
    var gen = this.artifactInfo.gen; // artifactConfig.gen property
    if (this.artifactInfo.isWizziModelArtifact()) {
...
Runner.prototype.runWizziModelArtifact = function (modelInfo, gen, callback) {
    ...
    if (modelInfo.isDirectory()) {
        modelInfos = modelInfo.getModelInfos();
    } else {
    ...
    AsyncModelLoader.loadMany(modelInfos, function (err, genContexts) {
        if (err) return callback(err, null); 

        AsyncArtifactGenerator.generate(
            self.artifactInfo, generator, modelInfos, genContexts, function (err) {
...
Runner.prototype.runWizziCollectionArtifact = function (modelInfo, gen, callback) {
    ...
    AsyncModelLoader.load(modelInfo, function (err, genCollectionContexts) {
        if (err) return callback(err, null);
        AsyncArtifactGenerator.generateFromCollectionContexts(
            self.artifactInfo, generator, modelInfo, genCollectionContexts, function (err) {
                callback(null, self.artifactInfo);
        });
...
Runner.prototype.runCodeWriteArtifact = function (contextInfos, gen, callback) {
    ...
    AsyncModelLoader.loadMany(contextInfos, function (err, genContexts) {
        if (err) return callback(err, null);

        AsyncArtifactGenerator.generateCodeWrite(
            self.artifactInfo, generator, genContexts, function (err) {
                callback(null, self.artifactInfo);
            });
...
/*
    ...
    without template transformation and generation.
    The source can be a folder, in this case a genWriter is created
...
Runner.prototype.runFinalArtifact = function (modelInfo, callback) {
    ...
    if (modelInfo.isDirectory()) {
        ...
        files.forEach(function (fileInfo) {
            ...
            // console.log('Runner.runFinalArtifact.item', modelInfo.src(), item.filepath);
            var genWriter = new GenWriter({
                model: null,
